...
`web/`にNext.js製のライブダッシュボードがあり、Vercelにデプロイ済みです（[usdjpy-fx-monitor.vercel.app](https://usdjpy-fx-monitor.vercel.app)）。判定・スコア内訳・ローソク足チャート（15分/30分/1時間足＋SMAオーバーレイ）・仮想ポジション成績・ニュース見出し・経済指標カレンダーを表示し、常時ライブ更新のレートティッカーと手動更新ボタンも備えています。詳細は[`web/README.md`](web/README.md)を参照してください。
...
├── web/                     # Next.js製ライブダッシュボード（web/README.md参照）
├── src/
...
│   ├── latest_snapshot.py  # ダッシュボード用data/latest.jsonの生成
│   ├── price_history.py    # ダッシュボード用ローソク足履歴（15m/30m/1h）の収集
│   └── main.py             # 実行エントリーポイント
├── data/
│   ├── latest.json                # 判定根拠・indicators・news・calendar（自動生成・自動更新）
│   └── price_history_{15m,30m,1h}.json  # ローリングOHLC履歴（自動生成・自動更新）
